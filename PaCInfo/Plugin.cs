...
class PickupPatch
{
	...
	private static double GetWeight(SellByWeight item, WeightDisplayType type) =>
		type switch
		{
			WeightDisplayType.Estimate => Math.Round(item.myWeight - UnityEngine.Random.Range(-0.3f, 0.3f), 2, MidpointRounding.AwayFromZero),
	...
	private static string EstSign(WeightDisplayType wType) => wType == WeightDisplayType.Estimate ? "~" : "";
	private static string EstSign(PriceDisplayType pType) => pType == PriceDisplayType.Estimate ? "~" : "";


	[HarmonyPostfix]
	[HarmonyPatch(typeof(CharPickUp), nameof(CharPickUp.CmdPickUpObject))]
	static void PickUpPatch(CharPickUp __instance, uint pickUpObject)
	{
